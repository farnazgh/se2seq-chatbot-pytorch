...
import gensim
model = gensim.models.KeyedVectors.load_word2vec_format('/wiki.txt', binary=False,unicode_errors='ignore' )
...
def prepareData(lang1, lang2, reverse=False):
    ...
    #get alll words
    lines = open('data/norm_fr-fr.txt', encoding='utf-8').read().strip().split('\n')
    all_pairs = [[s for s in l.split('\t')] for l in lines]
...
input_lang, output_lang, pairs = prepareData('fr', 'fr', False) 
...
#load stop wrods
def stopwords():
    lines = open('data/stopwords.txt', encoding='utf-8').read().strip().split('\n')
    return lines



def evaluateModelPrecision():
	#load test data set
    ...
    for line in pairs:
        input_ = line[0]
        target_ = line[1]
        output_words, attentions = evaluate(encoder1, attn_decoder1, input_)
        output_words = output_words[:-1]
        output_ =  ' '.join(output_words)
	...
	# cal word2vec simlarity
        ...
        target_wordlist = [s for s in target_.split(' ') if s in model and s not in swlist]
        output_wordlist = [s for s in output_.split(' ') if s in model and s not in swlist]
        
        for word in target_wordlist:
            target_w2v += model[word]
    ...
    word2vec_sim = (1 - word2vec_sim)*100
...
evaluateAndShowResults("چيکار کنم ؟")
evaluateAndShowResults("ببحشيد چي گفت ؟")

evaluateModelPrecision()
